...
def get_schema():
    ...
    schema['paths']['/accounts/{accountID}/instruments/{instrument}/candles']['get'][
...
def get_response_params(schema, parameters):
    if '$ref' not in parameters:
        ...
        for section_name, section in [('headers', parameters['headers']),
                                      ('schema', parameters['schema']['properties'])]:
...
def run():
    args = get_args()
    schema = get_schema()
    file_loader = FileSystemLoader('./tmpl')
    ...
    if args.type == 'request':
        ...
        writes.write(enum_tmpl.render(name=e['name'], comment=e['comment'], camel=e['camel'], variants=e['variants']))
    elif args.type == 'definition' and args.gen == 'struct':
        if args.url != 'all':
            e = get_definition_struct(schema, args.url)
            writes.write(struct_tmpl.render(name=e['name'], comment=e['comment'], camel=e['camel'], params=e['params']))
        else:
